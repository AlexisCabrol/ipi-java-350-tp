...
public class EmployeService {
     ...
     */
    public void embaucheEmploye(String nom, String prenom, Poste poste, NiveauEtude niveauEtude, Double tempsPartiel) throws Exception {
        logger.info("Tentative d'enregistrement d'un employé : {} {} {} {} {}", nom, prenom, poste, niveauEtude, tempsPartiel);
        ...
        if(lastMatricule == null){
            logger.info("Base vide => matricule initial choisi {}", Entreprise.MATRICULE_INITIAL);
            lastMatricule = Entreprise.MATRICULE_INITIAL;
        }
        //... et incrémentation
        Integer numeroMatricule = Integer.parseInt(lastMatricule) + 1;
        if(numeroMatricule >= 100000){
            logger.error("Dépassement du nombre limite de matricule {} !", numeroMatricule);
            throw new Exception("Dépassement du nombre limite de matricule");
        } else if (numeroMatricule > 90000){
            logger.warn("Attention, cap des matricules 90000 atteint !");
        }
        ...
        if(employeRepository.findByMatricule(matricule) != null){
            logger.error("L'employé de matricule " + matricule + " existe en BDD");
        ...
        if(tempsPartiel != null){
            ...
            logger.warn("Temps partiel absent sur l'employé !");
        }
        salaire = Math.round(salaire*100d)/100d;

        //Création et sauvegarde en BDD de l'employé.
        Employe employe = new Employe(nom, prenom, matricule, LocalDate.now(), salaire, Entreprise.PERFORMANCE_BASE, tempsPartiel);
        logger.info("Employé à sauvegardé : {}", employe.toString());
        try{
            employe = employeRepository.save(employe);
        }catch (Exception e) {
            logger.error("Employe non sauvegardé : {}", employe.toString());
        }
        logger.info("Employé sauvegardé : {}", employe.toString());
